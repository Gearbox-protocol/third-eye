...
func (repo *Repository) loadLastTreasuryTs() {
	data := core.DebtSync{}
	if err := repo.db.Raw("SELECT max(timestamp) AS last_calculated_at FROM treasury_snapshots").Find(&data).Error; err != nil {
		log.Fatal(err)
	}
...
func (repo *Repository) loadTreasurySnapshot() {
	...
	sql := `SELECT * FROM treasury_snapshots WHERE timestamp=0`
