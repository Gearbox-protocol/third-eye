...
func (mdl *CMv2) setv2AddrIfNotPresent() {
	// set credit manager
	cmContract, err := creditManagerv2.NewCreditManagerv2(common.HexToAddress(mdl.Address), mdl.Client)
	log.CheckFatal(err)
	mdl.cmContractv2 = cmContract
	// set credit facade contract
	mdl.facadeContractv2, err = creditFacade.NewCreditFacade(core.NULL_ADDR, nil)
	log.CheckFatal(err)
	//
