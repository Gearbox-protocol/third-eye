...
func (mgr *AccountTokenManager) AddTokenTransfer(tt *schemas.TokenTransfer) {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	if tt.IsFromAccount {
...
func (mgr *AccountTokenManager) CheckTokenTransfer(cm string, from, to int64) map[int64]map[string][]*schemas.TokenTransfer {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
...
func (mgr *AccountTokenManager) GetNoSessionTxs() (tts map[string][]*schemas.TokenTransfer) {
